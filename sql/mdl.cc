...
public:
  ...
  */
  ...
  void notify_conflicting_locks_if_needed(MDL_ticket *ticket)
  {
    if (m_strategy->needs_notification(ticket))
    {
      mysql_prlock_wrlock(&m_rwlock);
      notify_conflicting_locks(ticket->get_ctx());
      mysql_prlock_unlock(&m_rwlock);
    }
  }
    ...
    if (likely(m_strategy))
    {
      ...
      else
      {
        ...
        if (wait)
        {
          ...
          if (m_strategy->needs_notification(ticket))
            notify_conflicting_locks(mdl_context);
...
MDL_context::acquire_lock(MDL_request *mdl_request, double lock_wait_timeout)
{
  ...
  */
  ...
  while (cmp_timespec(abs_shortwait, abs_timeout) <= 0)
  {
    ...
    lock->notify_conflicting_locks_if_needed(ticket);
    set_timespec(abs_shortwait, 1);
  }
  if (wait_status == MDL_wait::EMPTY)
